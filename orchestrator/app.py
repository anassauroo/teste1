...
MOTION_BLUR = int(os.getenv("MOTION_BLUR", "5"))
...
# =========================
# Clipe pós-evento (Opção A)
# =========================
CLIP_ENABLED = os.getenv("CLIP_ENABLED", "0").strip() not in ("0", "false", "False", "FALSE")
CLIP_SECONDS = float(os.getenv("CLIP_SECONDS", "5"))
CLIP_FPS = int(os.getenv("CLIP_FPS", "15"))
CLIP_TIMEOUT_S = int(os.getenv("CLIP_TIMEOUT_S", "30"))
CLIP_COOLDOWN_S = float(os.getenv("CLIP_COOLDOWN_S", "15"))
CLIP_PRESET = os.getenv("CLIP_PRESET", "veryfast")
CLIP_CRF = int(os.getenv("CLIP_CRF", "28"))

# Corta o começo do clipe (para evitar segundos estáticos vindos de buffer)
CLIP_SKIP_HEAD_S = float(os.getenv("CLIP_SKIP_HEAD_S", "0"))

# Timeout do RTSP (microsegundos) — opção compatível para RTSP: -stimeout
# Ex.: 5000000 = 5s
CLIP_STIMEOUT_US = int(os.getenv("CLIP_STIMEOUT_US", "5000000"))
...
last_clip_ts: dict[str, float] = {}  # cooldown de clipe por stream
...
def record_clip_post_event(stream_path: str) -> Path | None:
    """
    Grava um clipe MP4 (pós-evento) a partir do RTSP por CLIP_SECONDS e salva em:
      /app/frames/<stream>/<YYYY-MM-DD>/clip_<HH-MM-SS_mmm>.mp4

    Ajustes de latência:
      - flags de low-latency
      - -stimeout (microsegundos) para RTSP
      - -ss após -i para cortar "cabeça" estática (CLIP_SKIP_HEAD_S), sem aumentar duração final
    """
    out_dir = FRAMES_DIR / _sanitize_path(stream_path) / _date_dir()
    out_dir.mkdir(parents=True, exist_ok=True)

    clip_file = out_dir / f"clip_{_ts_filename()}.mp4"
    rtsp_url = f"rtsp://{MEDIAMTX_HOST}:{MEDIAMTX_RTSP_PORT}/{stream_path}"
    ...
    cmd = [
        ...
        # low-latency / menos buffer
        "-fflags", "nobuffer",
        "-flags", "low_delay",
        "-analyzeduration", "0",
        "-probesize", "32",

        # timeout RTSP (microsegundos) — compatível
        "-stimeout", str(CLIP_STIMEOUT_US),

        "-i", rtsp_url,
    ]

    # corta o começo do clipe (após -i -> seek mais preciso em streams)
    if CLIP_SKIP_HEAD_S > 0:
        cmd += ["-ss", str(CLIP_SKIP_HEAD_S)]

    cmd += [
        # duração final desejada (não somar o skip)
        "-t", str(CLIP_SECONDS),
        "-r", str(CLIP_FPS),
        "-an",
        "-c:v", "libx264",
    ...
    try:
        subprocess.run(cmd, check=True, timeout=CLIP_TIMEOUT_S)
        return clip_file
    except subprocess.TimeoutExpired:
        print(f"[clip] timeout ao gravar clipe de '{stream_path}' ({CLIP_TIMEOUT_S}s)")
    except subprocess.CalledProcessError as e:
        print(f"[clip] falha ao gravar clipe de '{stream_path}': {e}")
    except Exception as e:
        print(f"[clip] erro inesperado ao gravar clipe de '{stream_path}': {e}")

    return None


async def publish_alert(stream_path: str, motion_ratio: float, event_frame_path: Path, clip_path: Path | None):
    """
    Publica alerta no Kafka (se habilitado), enriquecido com clip_path (se houver).
    """
    global producer
    ...
    payload = {
        ...
        "event_frame_path": str(event_frame_path),
        "clip_path": str(clip_path) if clip_path else None,
        "clip_seconds": CLIP_SECONDS if clip_path else None,
        "clip_skip_head_s": CLIP_SKIP_HEAD_S if clip_path else None,
        "date_utc": _date_dir(),
    }
...
async def capture_loop(stream_path: str):
    ...
    Loop por stream:
      ...
      - salva apenas eventos (ou tudo, se SAVE_ALL_FRAMES=1)
      - grava clipe MP4 pós-evento (Opção A) com cooldown próprio
      - publica alerta Kafka enriquecido com clip_path
    """
    print(f"[frames] iniciando loop para '{stream_path}' (intervalo={CAPTURE_INTERVAL_S}s)")

    last_event_ts.setdefault(stream_path, 0.0)
    last_clip_ts.setdefault(stream_path, 0.0)
    ...
    try:
        while not shutdown:
            ...
            if MOTION_ENABLED:
                ...
                if is_motion and can_emit:
                    ...
                    if CLIP_ENABLED:
                        can_clip = (now - last_clip_ts.get(stream_path, 0.0)) >= CLIP_COOLDOWN_S
                        if can_clip:
                            last_clip_ts[stream_path] = now
                            clip_path = await asyncio.to_thread(record_clip_post_event, stream_path)
                            if clip_path:
                                print(f"[clip] salvo: {clip_path}")
                            else:
                                print(f"[clip] falha ao gerar clipe para '{stream_path}'")

                    await publish_alert(stream_path, ratio, event_file, clip_path)
        ...
        last_clip_ts.pop(stream_path, None)
